...
//import("./globals.mjs").then((globals)=>{
  //const { fields } = await import("./form-fields.js");
  ...
  const FIELD_MAP = {
    ...
    "published-url":new fields.TextField("published",fields.validateAemPublishedUrl),
  ...
  CONTAINER.addEventListener("drop",(evt)=>{
    evt.preventDefault();
    var srcId = Number.parseInt(evt.dataTransfer.getData("text")),destId,
  ...
  function getBoundFieldset(fieldMap,site) {
    ...
    fields.getElementsByName(SET_COLOR_NAME,titleSection).forEach((colorButton)=>{
  ...
  function serializeFormData(container,fieldMap) {
    ...
    for (let fieldset of container.getElementsByClassName(FIELDSET_CLASS)) {
      ...
      for (let field in fieldMap) {
        serialized = fieldMap[field].fromInput(fields.getElementsByName(field,fieldset));
  ...
  function initOptionsPage(){
    ...
    .then((data) => {
      ...
      fields.sendDebugFeedback(`Loaded ${data.sites.length} items (index at ${envCount}).`);
  ...
  document.getElementById("export").addEventListener("click",(evt)=>{
    ...
    .then((data)=>{
      ...
      fields.download(JSON.stringify(toExport),"AEM-Developer-settings-export.json","application/JSON");
    })
  ...
  document.getElementById("import").addEventListener("click",(evt)=>{
    ...
    fileInput.addEventListener("change",(inputEvent)=>{
      ...
      .then((text)=>{
        ...
        .then(()=>{
          alert(`Settings imported (${sanitisedSettings.sites.length||0} sites)`);
        location.reload();
        })
        .catch((err)=>{
          console.error(err);
